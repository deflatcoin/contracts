...
//Example third-party contract interaction with sale maincontract without "out of gas fail" 
    ...
    if (playerPool.length >= rounds) {
      ...
      playerPool.length = 0;                
       ...
       if (playerPool.length == 1) {
           if (maincontract.call.gas(200000).value(address(this).balance)()) { emit Payout(this, maincontract, quota);} // Important call gas
       }
       reward = tokenReward.balanceOf(address(this))/((rounds+1)-playerPool.length);    
       if (reward > 0) { tokenReward.transfer(msg.sender, reward); }
    } 
